...
function prepGetRequest(option, req, res, callback){
  ...
  var pageInfo = {
    ...
    siteinfo: {protocol: req.protocol, host: req.hostname, path: decodeURIComponent(req.path), query: req.query, url: req.url},
    googleAnalyticsCode: systemVariable.getConfig('googleAnalyticsId'),
    other: {}
  };
  
/*****************TEST CODE to try and fix destination issues*******/
pageInfo.siteinfo.path = req.path;
/******************************************************************/
