...
var logger = {
  ...
  report: function(req, res, period, callback){
    ...
    function designReport(callback){
      ...
      designReport += '<table border="0" cellpadding="20" cellspacing="0" width="600" id="emailContainer">';
      designReport += '<tr>';
      designReport += '<td align="center" valign="top">';


      designReport += systemVariable.getConfig('siteName') +' activity report<br/><br/>';
      designReport += 'Start: ' + new Date(oldest).toString() + '<br/>';
      designReport += 'End: ' + new Date(now).toString() + '<br/><br/>';

      _.forEach(this.report, function(siteReport, siteName){
        designReport += '<h3>' + siteName + '</h3>';
...
//console.log('report: ', report);
            ...
            designReport += '<caption>' + level + '</caption>';
            designReport += '<tr>';
            ...
            designReport += '</tr>';
            designReport += '<tr>';
            designReport += '<td colspan="' + this.attributeArray.length + '">' + source + '</td>';
            designReport += '</tr>';
    ...
    function sendReport(callback){
      ...
      var body = this.htmlReport;
      //body += JSON.stringify(this.report, null, 2);
      console.log(body);
      //body += this.htmlReport;
