...
function runCron(req, res, callback){
  ...
  async.parallel(asyncFn, function(err, result){
    ...
    // console.log('result of all cron tasks:\n', result);
...
function backupDatabase(req, res, callback){
  ...
  var backupFilePath = process.cwd() + '/handy/tmp/';
...
var logger = {
  ...
  start: function(){
    ...
    this.path = path.join(process.cwd(), 'handy/logs/' + siteName.toLowerCase().replace(/ /g, '-') + '.log')
  ...
  report: function(req, res, period, callback){
    ...
    var buf = '';
    var line;
    this.report = {};  // will contain the final report as an object formated as {sitename: {level: {category: {other details}}}}
    ...
    function processLine(stringJSON){
      ...
      this.report[lineJSON.name] = this.report[lineJSON.name] || {}
      this.report[lineJSON.name][lineJSON.level] = this.report[lineJSON.name][lineJSON.level] || {};
      this.report[lineJSON.name][lineJSON.level][lineJSON.category] = this.report[lineJSON.name][lineJSON.level][lineJSON.category] || [];
      var tempObject = {};
      var tempArray = ['ip', 'url', 'method', 'referer', 'userId', 'msg', 'time', 'err'];  // list of attributes to record in report
      tempArray.forEach(function(val){
        if(lineJSON[val] !== undefined){tempObject[val] = lineJSON[val]}
      });
      this.report[lineJSON.name][lineJSON.level][lineJSON.category].push(tempObject);
      /*
      this.report[lineJSON.name][lineJSON.level][lineJSON.category].push({
        ip: lineJSON.ip || null, 
        url: lineJSON.url || null, 
        method: lineJSON.method || null, 
        referer: lineJSON.referer || null, 
        userId: lineJSON.userId || null, 
        msg: lineJSON.msg || null, 
        time: lineJSON.time || null, 
        error: lineJSON.err || null
      });*/
    }

    function sendReport(callback){
      //console.log('report: \n', utility.inspect(this.report));
      ...
      var body = systemVariable.getConfig('siteName') +' activity report\n\n';
      body += 'Start: ' + new Date(oldest).toString() + '\n';
      body += 'End: ' + new Date(now).toString() + '\n\n';
      body += JSON.stringify(this.report, null, 2);
