...
//var handyDirectory = utility.findHandyDirectory();  // doesn't work because utility is not fully loaded before system gets called
...
(function (){
  try {
    var fd = fs.openSync(handyDirectory + '/config/randomized-strings.js', 'r'); 
    secretStrings = require(handyDirectory + '/config/randomized-strings').randomized_strings; 
  }
  catch(err){
    ...
    text += "};\n\n";
    text += "exports.randomized_strings = randomizedStrings;"

    // write file
    fs.writeFileSync(handyDirectory + '/config/randomized-strings.js', text, {mode: 384});
  }
})();
  ...
  switch (err.status){
    case 403:
      prepGetRequest({
        ...
        res.status(403);
        logDetail = {type: 'warn', category: 'content', message: 'display 404 access denied page'};
...
function display(req, res, view, locals, logDetail){
   ...
   * [theme_title], [theme_description], [theme_canonical], [theme_sitename], [theme_login_logout], [theme_messages], [theme_ga]
   */
  ...
  function _tokenReplace(message){
    ...
    var translation = {
      ...
      'theme_login-logout': (locals.user.authenticated === true) ? '<a href="/logout">Logout</a>' : '<a href="/login">Login</a>',
