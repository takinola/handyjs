...
var utility = require('./utility')
  ...
  var prefix = secretStrings.redisPrefix + 'acl_';  // set a prefix that is unique for each site
  var acl = new Acl(new Acl.redisBackend(client, prefix));
...
function User(init, schema){
  ...
  var user_init = {
    ...
    role:[],
    organization: null, // for group managed accounts eg 'ClientCo'
    status: {individual: [], organization: []}  // tracks user status eg 'paid', 'suspended', etc.  status is also inherited from organization
  };
  ...
  var user_schema = {
    ...
    columns: {
      ...
      status: {type: 'VARCHAR', size: 512, null: true},
    }
  };
...
User.prototype.load = function(id, type, callback){
  system.genericLoad.bind(this, id, type, (function(err){
    ...
    // convert status from string back to array
    this.status = JSON.parse(this.status);

    // add in the inherited status from the company
    system.getOrganizationStatus.bind(this)()
    .then(function(){
...
/*
 * custom save function to remove organization status
 * from the user record prior to saving
 *
 * @api public
 */
User.prototype.save = function(callback){
  // remove status that is inherited from the user's organization
  this.status.organization = [];
  system.genericSave.bind(this, callback)();
}
    ...
    _populateNewUserDetails.bind(this)(req, requestingUser, (function(err){
      ...
      _checkUserAccountAlreadyExists.bind(this)((function(err, accountExistsFlag){
        ...
        if(accountExistsFlag){
          var error = new Error();
          error.type = 'account already exists';
  ...
  this.load(this.email, 'email', (function(err){
    ...
    this.createOneTimeLink('password', (function(err){
      ...
      this.save((function(err){
        ...
        var body = {text: system.tokenReplace(system.systemVariable.getConfig('password_recovery_body'), req, this)};
...
User.prototype.cancelAccount = function(req, callback){
  ...
  this.save((function(err){
    ...
    if(system.systemVariable.getConfig('account_cancelled_checkbox')){
      ...
      var body = {text: system.tokenReplace(system.systemVariable.getConfig('account_cancelled_body'), req, this)};     
      system.sendEmail(receipient, sender, subject, body, function(err, message){
        if(err){
...
/*
 * find content belonging to user organization
 * this is a convenience function to wrap around content.findContent ensuring
 * content does not leak across organizations
 *
 ...
 * @api public
 */
User.prototype.findSharedOrganizationContent = function(type, criteria, options, callback){
  // add user organization to search options
  options.organization = this.organization;

  content.findContent(type, criteria, options, function(err, results){
...
function _populateNewUserDetails(req, requestingUser, callback){
  // minimum requirements is for an email address to be provided
  ...
  this.lastlogin = requestingUser.id ? null : new Date();  // set last login if the user is self-registering
  this.createdate = new Date();
  
  // check if any additions have been added e.g. organization and roles
  if(typeof req.body._additions !== 'undefined'){
    this.organization = parseInt(req.body._additions.organization)  || null;
    this.role = req.body._additions.role || [];
  }
...
function _sendRegistrationNotifications(req, callback){
  ...
  switch (system.systemVariable.getConfig('registerAuthority').toLowerCase()){
    case 'administrator':
      ...
      body = {text: system.tokenReplace(system.systemVariable.getConfig('welcome_new_user_admin_body'), req, this)};
      break;
    case 'visitor':
      ...
      if(this.didNotChooseOwnPassword){
        ...
        switch (this.verified){
          case true:
            ...
            body = {text: system.tokenReplace(system.systemVariable.getConfig('welcome_new_user_no_approval_required_body'), req, this)};
            break;
          case false:
            ...
            body = {text: system.tokenReplace(system.systemVariable.getConfig('welcome_new_user_email_verification_required_body'), req, this)};
            break;
        }
    ...
    case 'requireadmin':
      ...
      body = {text: system.tokenReplace(system.systemVariable.getConfig('welcome_new_user_approval_required_body'), req, this)};
      break;
  }
              ...
              */
              postUserVerificationProcessing([this.id], (function(err){
                ...
                if(system.systemVariable.getConfig('account_activation_checkbox')){
                  ...
                  var body = {text: system.tokenReplace(system.systemVariable.getConfig('account_activation_body'), req, this)};
 ...
 * @param {string} contentIdType - type of of content id e.g. 'url', 'id', etc
...
function checkUserHasSpecificContentPermission(req, res, uid, contentType, contentId, contentIdType, actionType, callback){
  ...
  checkPermission(uid, resource, [taskGeneral], function(err, result){
    ...
    if(!result){
      checkPermission(uid, resource, [taskSpecific], function(err, result2){
        ...
        newContent.load(contentId, contentIdType, (function(err){
