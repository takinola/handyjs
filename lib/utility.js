...
/*
 * trim space around all entries in a form
 *
...
function trimFormTextEntries(form){
  _.forEach(form, function(entry, field){
    typeof entry === 'string' ? form[field] = entry.trim() : form[field] = form[field];
  });
 ...
 * checks if the email has format <something>@<something
 *
 ...
 * checks if the url has format http://* or https://*
 *
...
function validateUrl(url){
  let urlRegexp = new RegExp("https?://.+"); // validates pattern http://* or https://*
