...
/*
 * Prepare destination url to be passed into form POST content/:type/:id:/:action
 * This function replaces certain UTF-8 encoded characters because Express decodes
 * them improperly(?) when the destination is retrieved from req.query.destination
 * The function is used to either encode the URI or decode it
 *
 * @param {string} uri - uri to be modified for insertion into destination query
 * @param {string} type - specify if the function is "encode" or "decode" the uri
 * @api public
 */
exports.prepDestinationUri = prepDestinationUri;

function prepDestinationUri(uri, type){
  // end processing and return null if empty uri
  if (uri === undefined || uri === '' || uri === null){
    uri = null;
    return uri;
  }

  // list of ut-8 characters to be replaced
  var replacement = {
    '%3F': 'qm',
    '%3D': 'eq',
    '%23': 'hs',
  ...
  var fix = '----';  // string pre- and suf- fix
  var re; // regex for replacements
  switch (type.toLowerCase()){
    case 'encode':
      _.forEach(replacement, function(newText, oldText){
        re = new RegExp(oldText, "gi");
        uri = uri.replace(re, fix + newText + fix);
      });
      return uri;
      break;
    case 'decode':
      ...
      _.forEach(replacement, function(newText, oldText){
        re = new RegExp(fix + newText + fix, 'g');
        uri = uri.replace(re, oldText);
      });
      return uri;
      break;
  } 
