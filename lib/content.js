...
var secretStrings = system.secretStrings;
...
Content.prototype.save = function(callback){
  ...
  function _invalidateCache(asyncCallback){
    var cacheKey = secretStrings.redisPrefix + this.contenttype + '_' + this.id;
...
Content.prototype.load = function(id, type, callback){
  var cacheKey = secretStrings.redisPrefix + this.contenttype + '_' + id;
...
function submitXmlSitemap(req, res, callback){
  ...
  if(!system.systemVariable.getConfig('sitemapSubmit')){
    return callback(null, {err: null, message: 'XML sitemap not submitted by admin request'});
  }
